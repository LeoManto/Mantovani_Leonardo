...
public class TestMoveVirtualRobot {
    ...
    public void testMoveForwardNoHit() {
        ...
        boolean moveFailed = appl.moveForward(400);
        assertTrue( ! moveFailed  );
        moveFailed = appl.moveBackward(400);  //back to home
        assertTrue( ! moveFailed  );
    }

    @Test public void testMoveForwardHit() {
        System.out.println("TestMoveVirtualRobot | testMoveForward ");
    ...
    @Test public void testBoundaryWalk(){
        System.out.println("TestMoveVirtualRobot | BoundaryWalk");
        String moves = "";
        int i = 0;
        while(i != 4){
            boolean moveFailed = appl.moveForward(200);
            moves += "w";
            if(!moveFailed){
                assertTrue(!moveFailed);
            } else {
                assertTrue(moveFailed);
                moveFailed = appl.moveLeft(200);
                assertTrue(!moveFailed);
                moves += "l ";
                i++;
            }
    ...
    */
    ...
    @Test public void prova(){
        boolean complete = appl.boundaryWalk(200);
        assertTrue(complete);
    }
