	...
	suspend fun doRead(   ){
		...
		while( true ){
				...
				if( data != null ){
					try{ 
						val v = data.toInt()
						if( v <= 50 && occupato == false){	//A first filter ...
							occupato = true
							val m1 = "distance( ${v*2} )"
						...
						}else if(v > 50 && occupato == true){
								occupato = false
								val m2 = "distance( ${v*2} )"
								val event = MsgUtil.buildEvent( "sonarHCSR04Support","sonar",m2)
								//emit( event )  //should be propagated also to the remote resource
								emitLocalStreamEvent( event )		//not propagated to remote actors
								println("sonarHCSR04Support doRead ${counter++}: $event "   )	
						}
				...
				//delay( 500 ) 	//Avoid too fast generation
 		}
		}
	}
}